...
Func Example()
	...
	_midi_SelectPatch($hDevice, $iChannel, $PGM_PAD_POLYSYNTH)
	PlayNote($hDevice, $iChannel)

	MsgBox(0, "GS/XG Filter Params", "Press Ok to change the release time. (if supported)")
	_midi_SetADSR_Release($hDevice, $iChannel, 0x60)
	PlayNote($hDevice, $iChannel)

	_midi_SetADSR_Release($hDevice, $iChannel)
...
Func PlayNote($hDevice, $iChannel)
