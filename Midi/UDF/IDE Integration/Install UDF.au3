...
Func Install()
	...
	Local Const $sOutUDFFile = "midi.au3"
	Local Const $sOutUDFConstsFile = "midiConstants.au3"

	Local $sNewCallTips = FileRead("midiCallTips.txt")
	Local $sNewKeywords = FileRead("midiKeywords.txt")
	Local $sUDF = FileRead("..\midi.au3")
	Local $sUDFConstants = FileRead("..\midiConstants.au3")
	...
	Else
		...
		While 1
			...
			If StringLeft($sLine, 6) <> "_midi_" Then $sCurCallTips &= $sLine
		WEnd
	...
	Else
		...
		While 1
			...
			If StringLeft(StringStripWS($sLine, 1), 1) = ";" Then
				...
				ContinueLoop
			EndIf
			$sLine = StringRegExpReplace($sLine, "_midi_[[:graph:]]+", "")
