...
Func Example()
	...
	MsgBox(0, "GS/XG Filter Params", "Press Ok to sweep the filter up. (if supported)")
	For $i = 0x40 To 0x7F
		_midi_SetFilterCutoff($hDevice, $iChannel, $i)
		Sleep(10)
	Next
	Sleep(1500)

	_midi_SendNoteOff($hDevice, $iChannel, 0x3C)
	_midi_SetFilterCutoff($hDevice, $iChannel)
