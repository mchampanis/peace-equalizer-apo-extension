...
Func Example()
	...
	MsgBox(0, "GS/XG Drum Params", "Press Ok to change the drum's chorus send level (if supported)")
	For $i = 0 To 0x7F Step 0x20
		_midi_SetDrumChorus($hDevice, $iChannel, $NTE_SNARE, $i)
		PlayDrum($hDevice, $NTE_SNARE)
	Next
	Sleep(500)

	_midi_SetDrumChorus($hDevice, $iChannel, $NTE_SNARE)
