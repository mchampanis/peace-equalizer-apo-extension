...
Func Example()
	...
	Local $hGUI, $hDevice
	Local Const $XG_A_SYSON = Binary("0x00007E")

	_midi_Startup()
	$hGUI = GUICreate("dummy")
	$hDevice = _midi_OpenOutput(SelDevice(), $hGUI)

	MsgBox(0, "Yamaha XG Parameter Change", "Press Ok to send a 'XG System On' message")
	_midi_SendXGParamChange($hDevice, $XG_A_SYSON, Chr(0))
	MsgBox(0, "Yamaha XG Parameter Change", "Message Sent")
	Sleep(500)
