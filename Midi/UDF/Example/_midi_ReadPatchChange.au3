...
Func Example()
	...
	If Not @error Then
		...
		While 1
			...
			If Not @error Then
				$aPatch = _midi_ReadPatchChange($hDevice, $aMsg)
				If Not @error Then
					$sLine = StringFormat("Patch: chan[%s] program[%3d] BankMSB[0x%02X] BankLSB[0x%02X] \r\n", _
							$aMsg[1], $aPatch[0], $aPatch[1], $aPatch[2])
