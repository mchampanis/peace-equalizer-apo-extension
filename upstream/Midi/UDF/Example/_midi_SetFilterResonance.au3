...
Func Example()
	...
	_midi_SelectPatch($hDevice, $iChannel, $PGM_PAD_WARM)
	_midi_SendNoteOn($hDevice, $iChannel, 0x3C, 0x60)

	MsgBox(0, "GS/XG Filter Params", "Press Ok to increase the filter resonance (if supported)")
	For $i = 0x40 To 0x7F
		_midi_SetFilterResonance($hDevice, $iChannel, $i)
		Sleep(10)
	Next
	Sleep(1500)

	_midi_SendNoteOff($hDevice, $iChannel, 0x3C)
	_midi_SetFilterResonance($hDevice, $iChannel)
